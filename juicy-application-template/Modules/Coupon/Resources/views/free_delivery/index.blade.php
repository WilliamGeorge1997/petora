...
@section('content')
        ...
        <div class="card">
            <div
                class="
                    ...
                    ">
                <div>
                    <h4 class="card-title mb-25"> فلترة التوصيل مجانا
                    </h4>
                ...
                <div class="card-body">
                    <form class="dt_adv_search">
                        <div class="row g-1 mb-md-1">
                            ...
                            <div class="col-md-4">
                                <label class="form-label" for="basic-icon-default-date"> الشركة</label>

                                <select name="company_id" class="select2 form-select" id="company_id"
                                    onchange="ajax_branches(this.value)">
                                    <option selected value="{{ null }}">اختر</option>

                                    @foreach ($companies as $company)
                                        <option @if ($company->id == @$_GET['company_id']) selected @endif
                            ...
                            <div class="col-md-4">
                                ...
                                <select name="branch_id" class="select2 form-select branch_list" id="name511">
                                    <option selected value="{{ null }}">اختر</option>
    ...
    <!--/ Basic table -->
    {{ $freeDelivery->withQueryString()->links() }}
@endsection
...
@section('js')
    ...
    <script>
        $(document).ready(function() {
            ...
            if (company_id != null) {
                ajax_branches(company_id);
            }
        })

        function ajax_branches(val) {
            console.log(val);
            var baseURL = "{{ url('admin/branches') }}"; // Laravel URL helper to get the base URL
            var queryURL = baseURL + "?company_id=" + val; // Append 'company_id' query parameter
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    $('.branch_list').empty();
                    $('.branch_list').append('<option value="" disable="true"  selected="true">اختر</option>');
                    $.each(data['data'], function(index, villagesObj) {
                        var isSelected = villagesObj.id == branch_id ? ' selected="selected"' : '';
                        $('.branch_list').append('<option  value="' + villagesObj.id + '"' +
                            isSelected + '>' +
                            villagesObj.title['ar'] + '</option>');
                    })
                },
    ...
    <script>
        $(function() {
            ...
            if (company_id != null) {
                ajaxRequest += "company_id=" + company_id + '&';
            }
                        ...
                        },
                        {
                            ...
                            render: function(data, type, full, meta) {
                                ...
                                return (
                                    ...
                                    @can('Edit-freeDelivery')
                                        +
                                        '<a href="freeDelivery/activate/' + data +
                                            '" class="dropdown-item">' +
                        ...
                        @can('Create-freeDelivery')
...

                            {
                                ...
                                action: function(e, dt, node, config) {
                                    ...
                                    window.location.href = './freeDelivery/create';
                                },
                                init: function(api, node, config) {
                                    $(node).removeClass('btn-secondary');
                                }
            ...
            $('.datatables-basic tbody').on('click', '.delete-record', function() {
                ...
                Swal.fire({
                    ...
                    $.ajax({
                        url: window.location.origin + "/admin/freeDelivery/" + id,
                        type: 'POST',
                        data: {
                            "id": id,
                            "_method": "DELETE",
