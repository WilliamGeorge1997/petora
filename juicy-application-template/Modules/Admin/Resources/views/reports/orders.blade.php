                  ...
                  <form class="dt_adv_search" >
                                    ...
                                    <div style="margin-top: 5%"  class="col-md-3">
                                        ...
                                        <select  name="order_method_id"  class="form-select" id="name5221">
                                            ...
                                            @foreach ( $viewModel->orderMethod()  as $orderMethodsData)
                                                <option @if($orderMethodsData['id'] == @$_GET['order_method_id']) selected @endif  value="{{ $orderMethodsData['id'] }}">{{ $orderMethodsData['title'] }}</option>
                                            @endforeach
                                 ...
                                 <option selected  value="{{ null }}">الكل</option>
                                        ...
                                        @foreach ( $viewModel->paymentMethod()  as $paymentMethodData)
                                            <option  @if($paymentMethodData['id'] == @$_GET['payment_method_id']) selected @endif  value="{{ $paymentMethodData['id'] }}">{{ $paymentMethodData['title'] }}</option>
                                        @endforeach
                                    ...
                                    <select  name="order_status_id"   class="form-select" id="name511">
                                       <option selected  value="{{ null }}">الكل</option>
                                        ...
                                        @foreach ($viewModel->orderStatus() as $orderStatusData)
                                            <option  @if($orderStatusData['id'] == @$_GET['order_status_id']) selected @endif value="{{ $orderStatusData['id'] }}">{{ $orderStatusData['title'] }}</option>
                                        @endforeach
                                            ...
                                            <select  name="branch_id"  class="form-select" id="name5">
                                               <option selected  value="{{ null }}">الكل</option>
                                                ...
                                                @foreach ( $viewModel->Branches() as $Branch)
                                                    <option  @if($Branch['id'] == @$_GET['branch_id']) selected @endif value="{{ $Branch['id'] }}">{{ $Branch['title'] }}</option>
                                                @endforeach
        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header">
                    ...
                    <a  href="{{ url("export-orderreport/xlsx?$Parameters") }}"> <button type="button"  style="float:left" class="btn btn-outline-primary ">
