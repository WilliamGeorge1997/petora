            ...
            <div class="col-sm-12">
                <div class="card">
                    <div class="card-header">
                        <h4 class="card-title">{{ __('store::general.products') ?? 'Products' }} - {{ $store->title }}</h4>
                    </div>
                    <div class="card-body">
                        <nav aria-label="breadcrumb">
                            <ol class="breadcrumb">
                                <li class="breadcrumb-item"><a href="{{ route('admin.dashboard') }}">{{ __('store::general.home') ?? 'Home' }}</a></li>
                                <li class="breadcrumb-item"><a href="{{ route('admin.store.index') }}">{{ __('store::general.stores') ?? 'Stores' }}</a></li>
                                <li class="breadcrumb-item active" aria-current="page">{{ __('store::general.products') ?? 'Products' }}</li>
                            </ol>
                        </nav>
                    </div>
                </div>
            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-header border-bottom p-1">
                        <div class="head-label"><h6 class="mb-0">{{ __('store::general.products') ?? 'Products' }}</h6></div>
                        <div class="dt-action-buttons text-end">
                            <div class="dt-buttons d-inline-flex">
                                <button class="dt-button btn btn-outline-primary me-2" type="button" data-bs-toggle="modal" data-bs-target="#importModal">
                                    <span><i data-feather="upload"></i> {{ __('store::general.import') ?? 'Import Excel' }}</span>
                                </button>
                                <a href="{{ route('admin.store.products.export', $store->id) }}" class="dt-button btn btn-outline-success">
                                    <span><i data-feather="download"></i> {{ __('store::general.export') ?? 'Export Excel' }}</span>
                                </a>
                            </div>
                        </div>
                    </div>
