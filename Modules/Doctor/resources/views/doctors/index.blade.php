            ...
            <div class="col-sm-12">
                <div class="card">
                    <div class="card-header">
                        <h4 class="card-title">{{ __('doctor::general.doctors') ?? 'Doctors' }}</h4>
                    </div>
                    <div class="card-body">
                        <nav aria-label="breadcrumb">
                            <ol class="breadcrumb">
                                <li class="breadcrumb-item"><a
                                        href="{{ route('admin.dashboard') }}">{{ __('doctor::general.home') ?? 'Home' }}</a></li>
                                <li class="breadcrumb-item active" aria-current="page">
                                    {{ __('doctor::general.doctors') ?? 'Doctors' }}</li>
                            </ol>
                        </nav>
                    </div>
                </div>
    ...
    <section class="card mb-2">
        <div class="card-body">
            <form>
                <div class="row g-2">
                    <div class="col-md-4">
                        <label class="form-label">{{ __('doctor::attribute.name') }}</label>
                        <input type="text" name="name" class="form-control"
                            placeholder="{{ __('doctor::attribute.name') }}" value="{{ request('name') }}">
                    </div>
                    <div class="col-md-3">
                        <label class="form-label">{{ __('doctor::attribute.is_active') }}</label>
                        <select name="is_active" class="form-select">
                            <option value="">{{ __('doctor::general.select_status') ?? 'الكل' }}</option>
                    ...
                    <div class="col-md-4 d-flex align-items-end">
                        <button type="submit" class="btn btn-primary me-1">{{ __('doctor::general.search') ?? 'Search' }}</button>
                        <a href="{{ route('admin.doctor.index') }}"
                            class="btn btn-outline-secondary">{{ __('doctor::general.reset') ?? 'Reset' }}</a>
                    </div>
                </div>
            </form>
        </div>
                    ...
                    <table class="datatables-basic table">
                        <thead>
                            <tr>
                                <th></th>
                                <th></th>
                                <th>{{ __('doctor::attribute.id') ?? 'ID' }}</th>
                                <th>{{ __('doctor::attribute.name') }}</th>
                                <th>{{ __('doctor::attribute.specialty') }}</th>
                                <th>{{ __('doctor::attribute.created_at') ?? 'Created At' }}</th>
                                <th>{{ __('doctor::attribute.is_active') }}</th>
                                <th></th>
                            </tr>
                        </thead>
                    </table>
    ...
    <script>
        $(function() {
            ...
            if (dt_basic_table.length) {
                var dt_basic = dt_basic_table.DataTable({
                    ...
                    columnDefs: [{
                        ...
                        },
                        {
                            // Actions
                            targets: -1,
                            title: '{{ __('doctor::general.actions') }}',
                            orderable: false,
                            render: function(data, type, full, meta) {
                                return (
                                    ...
                                    @can('Delete-doctor')
                                        ...
                                        feather.icons['trash-2'].toSvg({
                                            ...
                                            '{{ __('doctor::general.delete') ?? 'Delete' }}</a>'
                                    @endcan +
                                    '</div>' +
                                    '</div>'
                                    @can('Edit-doctor')
                    ...
                    responsive: {
                        details: {
                            display: $.fn.dataTable.Responsive.display.modal({
                                header: function(row) {
                                    var data = row.data();
                                    return 'Details of ' + data.name.{{ $locale }};
                                }
                            }),
                            type: 'column',
                ...
                $('div.head-label').html('<h6 class="mb-0">{{ __('doctor::general.main_data') ?? 'Data' }}</h6>');
            }
            ...
            $('.datatables-basic tbody').on('click', '.delete-record', function() {
                ...
                var id = dt_basic.row($(this).parents('tr')).data().id;
                Swal.fire({
                    title: '{{ __('doctor::general.sure_delete') ?? 'Are you sure?' }}',
                    text: '{{ __('doctor::general.cant_revert') ?? 'You cannot revert this!' }}',
                    icon: 'warning',
                    showCancelButton: true,
                    confirmButtonText: '{{ __('doctor::general.yes_delete') ?? 'Yes, delete!' }}',
                    cancelButtonText: '{{ __('doctor::general.cancel') ?? 'Cancel' }}',
                    customClass: {
