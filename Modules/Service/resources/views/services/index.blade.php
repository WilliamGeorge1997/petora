            ...
            <div class="col-sm-12">
                <div class="card">
                    <div class="card-header">
                        <h4 class="card-title">{{ __('service::general.services') }}</h4>
                    </div>
                    <div class="card-body">
                        <nav aria-label="breadcrumb">
                            <ol class="breadcrumb">
                                <li class="breadcrumb-item"><a
                                        href="{{ route('admin.dashboard') }}">{{ __('service::general.home') }}</a></li>
                                <li class="breadcrumb-item active" aria-current="page">
                                    {{ __('service::general.services') }}</li>
                            </ol>
                        </nav>
                    </div>
                </div>
    ...
    <section class="card mb-2">
        <div class="card-body">
            <form>
                <div class="row g-2">
                    <div class="col-md-4">
                        <label class="form-label">{{ __('service::attribute.title') }}</label>
                        <input type="text" name="title" class="form-control"
                            placeholder="{{ __('service::attribute.title') }}" value="{{ request('title') }}">
                    </div>
                    <div class="col-md-3">
                        <label class="form-label">{{ __('service::attribute.is_active') }}</label>
                        <select name="is_active" class="form-select">
                            <option value="">{{ __('service::general.select_status') ?? 'الكل' }}</option>
                    ...
                    <div class="col-md-4 d-flex align-items-end">
                        <button type="submit" class="btn btn-primary me-1">{{ __('service::general.search') }}</button>
                        <a href="{{ route('admin.service.index') }}"
                            class="btn btn-outline-secondary">{{ __('service::general.reset') }}</a>
                    </div>
                </div>
            </form>
        </div>
                    ...
                    <table class="datatables-basic table">
                        <thead>
                            <tr>
                                ...
                                <th>{{ __('service::attribute.created_at') }}</th>
    ...
    <script>
        $(function() {
            ...
            if (dt_basic_table.length) {
                var dt_basic = dt_basic_table.DataTable({
                    ...
                    columnDefs: [{
                        ...
                        },
                        {
                            ...
                            render: function(data, type, full, meta) {
                                var $user_img = full['image_url'], // Use image_url or image based on model setup, default to image
                                    $name = data.{{ $locale }};
                                if (!$user_img && full['image']) {
                                    $user_img = full['image']; // Fallback
                                }
                                if ($user_img) {
                                    // For Avatar image
                                    var $output =
                        ...
                        },
                        {
                            // Actions
                            targets: -1,
                            title: '{{ __('service::general.actions') }}',
                            orderable: false,
                            render: function(data, type, full, meta) {
                                return (
                ...
                $('div.head-label').html('<h6 class="mb-0">{{ __('service::general.main_data') }}</h6>');
            }
            ...
            $('.datatables-basic tbody').on('change', '.change-status', function() {
                ...
                $.ajax({
                    url: '/admin/services/' + id + '/activate',
                    type: 'PATCH',
                    data: {
                        _token: token
                    }
            ...
            $('.datatables-basic tbody').on('click', '.delete-record', function() {
                ...
                var id = dt_basic.row($(this).parents('tr')).data().id;
                Swal.fire({
                    title: '{{ __('service::general.sure_delete') }}',
                    text: '{{ __('service::general.cant_revert') }}',
                    icon: 'warning',
                    showCancelButton: true,
                    confirmButtonText: '{{ __('service::general.yes_delete') }}',
                    cancelButtonText: '{{ __('service::general.cancel') }}',
                    customClass: {
