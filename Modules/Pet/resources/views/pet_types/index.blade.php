...
@section('content')
            ...
            <div class="col-sm-12">
                <div class="card">
                    <div class="card-header">
                        <h4 class="card-title">{{ __('pet::general.pet_types') }}</h4>
                    </div>
                    <div class="card-body">
                        <nav aria-label="breadcrumb">
                            <ol class="breadcrumb">
                                <li class="breadcrumb-item"><a
                                        href="{{ route('admin.dashboard') }}">{{ __('pet::general.home') }}</a></li>
                                <li class="breadcrumb-item active" aria-current="page">
                                    {{ __('pet::general.pet_types') }}</li>
                            </ol>
                        </nav>
                    </div>
                </div>
    ...
    <section class="card mb-2">
        <div class="card-body">
            <form>
                <div class="row g-2">
                    <div class="col-md-4">
                        <label class="form-label">{{ __('pet::attribute.title') }}</label>
                        <input type="text" name="title" class="form-control"
                            placeholder="{{ __('pet::attribute.title') }}" value="{{ request('title') }}">
                    </div>
                    <div class="col-md-3">
                        <label class="form-label">{{ __('pet::attribute.is_active') }}</label>
                        <select name="is_active" class="form-select">
                            <option value="">{{ __('pet::general.select_status') ?? 'الكل' }}</option>
                    ...
                    <div class="col-md-4 d-flex align-items-end">
                        <button type="submit" class="btn btn-primary me-1">{{ __('pet::general.search') }}</button>
                        <a href="{{ route('admin.pet_type.index') }}"
                            class="btn btn-outline-secondary">{{ __('pet::general.reset') }}</a>
                    </div>
                </div>
            </form>
        </div>
                    ...
                    <table class="datatables-basic table">
                        <thead>
                            <tr>
                                <th></th>
                                <th></th>
                                <th>{{ __('pet::attribute.id') }}</th>
                                <th>{{ __('pet::attribute.title') }}</th>
                                <th>{{ __('pet::attribute.created_at') }}</th>
                                <th>{{ __('pet::attribute.is_active') }}</th>
                                <th></th>
                            </tr>
                        </thead>
                    </table>
    ...
    <!--/ Basic table -->
    {{ $petTypes->withQueryString()->links() }}
@endsection
    ...
    <script>
        $(function() {
            ...
            if (dt_basic_table.length) {
                                ...
                                return (
                                    ...
                                    @can('Delete-pettype')
                                        ...
                                        feather.icons['trash-2'].toSvg({
                                            ...
                                            '{{ __('pet::general.delete') }}</a>'
                                    @endcan +
                                    '</div>' +
                                    '</div>'
                                    @can('Edit-pettype')
                                        +
                                        '<a href="pet-types/' + data +
                ...
                $('div.head-label').html('<h6 class="mb-0">{{ __('pet::general.main_data') }}</h6>');
            }
            ...
            $('.datatables-basic tbody').on('change', '.change-status', function() {
                ...
                $.ajax({
                    url: '/admin/pet-types/' + id + '/activate',
                    type: 'PATCH',
                    data: {
                        _token: token
                    }
            ...
            $('.datatables-basic tbody').on('click', '.delete-record', function() {
                ...
                var id = dt_basic.row($(this).parents('tr')).data().id;
                Swal.fire({
                    title: '{{ __('pet::general.sure_delete') }}',
                    text: '{{ __('pet::general.cant_revert') }}',
                    icon: 'warning',
                    showCancelButton: true,
                    confirmButtonText: '{{ __('pet::general.yes_delete') }}',
                    cancelButtonText: '{{ __('pet::general.cancel') }}',
                    customClass: {
