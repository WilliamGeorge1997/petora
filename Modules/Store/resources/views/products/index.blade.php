...
@section('title', __('store::general.products'))
            ...
            <div class="col-sm-12">
                <div class="card">
                    <div class="card-header">
                        <h4 class="card-title">{{ __('store::general.products') }} - {{ $store->getTranslation('title', $locale) }}</h4>
                    </div>
                    <div class="card-body">
                        <nav aria-label="breadcrumb">
                            <ol class="breadcrumb">
                                <li class="breadcrumb-item"><a
                                        href="{{ route('admin.dashboard') }}">{{ __('store::general.home') }}</a></li>
                                <li class="breadcrumb-item"><a
                                        href="{{ route('admin.store.index') }}">{{ __('store::general.stores') }}</a></li>
                                <li class="breadcrumb-item active" aria-current="page">
                                    {{ __('store::general.products') }}</li>
                            </ol>
                        </nav>
                    </div>
                </div>
    ...
    <section class="card mb-2">
        <div class="card-body">
            <form>
                <div class="row g-2">
                    <div class="col-md-4">
                        <label class="form-label">{{ __('store::attribute.title') }}</label>
                        <input type="text" name="title" class="form-control"
                            placeholder="{{ __('store::attribute.title') }}" value="{{ request('title') }}">
                    </div>
                    <div class="col-md-3">
                        <label class="form-label">{{ __('store::attribute.is_active') }}</label>
                        <select name="is_active" class="form-select">
                            <option value="">{{ __('store::general.select_status') ?? 'الكل' }}</option>
                    ...
                    <div class="col-md-4 d-flex align-items-end">
                        <button type="submit" class="btn btn-primary me-1">{{ __('store::general.search') }}</button>
                        <a href="{{ route('admin.store.products.index', $store->id) }}"
                            class="btn btn-outline-secondary">{{ __('store::general.reset') }}</a>
                    </div>
                </div>
            </form>
        </div>
                    ...
                    <table class="datatables-basic table">
                        <thead>
                            <tr>
                                <th></th>
                                <th></th>
                                <th>{{ __('store::attribute.id') }}</th>
                                <th>{{ __('store::attribute.title') }}</th>
                                <th>{{ __('product::attribute.category_id') ?? 'Category' }}</th>
                                <th>{{ __('product::attribute.price') ?? 'Price' }}</th>
                                <th>{{ __('store::attribute.is_active') }}</th>
                                <th></th>
                            </tr>
                        </thead>
                    </table>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body px-sm-5 pb-5">
                    <div class="text-center mb-2">
                        <h1 class="mb-1">{{ __('store::general.import') ?? 'Import Products' }}</h1>
                    </div>
                    <form method="POST" action="{{ route('admin.store.products.import', $store->id) }}" enctype="multipart/form-data">
                        @csrf
                        <div class="mb-1">
                            <label class="form-label" for="file">{{ __('store::general.excel_file') ?? 'Excel File' }}</label>
                            <input type="file" id="file" name="file" class="form-control" accept=".xlsx, .xls, .csv" required />
                        </div>
                        <button type="submit" class="btn btn-primary w-100 mt-2">{{ __('store::general.submit') ?? 'Submit' }}</button>
                    </form>
                </div>
            </div>
        </div>
...
@section('js')
    ...
    <script>
        $(function() {
            ...
            var ajaxRequest = "{{ route('admin.store.products.index', $store->id) }}?";
            ...
            if (dt_basic_table.length) {
                var dt_basic = dt_basic_table.DataTable({
                    ...
                    columnDefs: [{
                        ...
                        },
                        {
                            // Actions
                            targets: -1,
                            title: '{{ __('store::general.actions') }}',
                            orderable: false,
                            render: function(data, type, full, meta) {
                                var editUrl = "{{ route('admin.store.products.edit', [$store->id, ':id']) }}".replace(':id', data);
                                return (
                    ...
                    buttons: [
                        {
                            ...
                            }) + '{{ __('store::general.import_all') }}',
                            className: 'btn btn-outline-info me-1',
                            action: function(e, dt, node, config) {
                                var form = $(
                                    '<form method="POST" action="{{ route('admin.store.products.import-all', $store->id) }}" style="display:none;">' +
                                    '{{ csrf_field() }}' +
                                    '</form>');
                                $('body').append(form);
                                form.submit();
                        ...
                        },
                        {
                            text: feather.icons['upload'].toSvg({
                                class: 'me-50 font-small-4'
                            }) + '{{ __('store::general.import') ?? 'Import Excel' }}',
                        ...
                        },
                        {
                            text: feather.icons['download'].toSvg({
                                class: 'me-50 font-small-4'
                            }) + '{{ __('store::general.export') ?? 'Export Excel' }}',
                            className: 'btn btn-outline-success',
                            action: function(e, dt, node, config) {
                                window.location.href = "{{ route('admin.store.products.export', $store->id) }}";
                            },
                            init: function(api, node, config) {
                                $(node).removeClass('btn-secondary');
                            }
                ...
                $('div.head-label').html('<h6 class="mb-0">{{ __('store::general.products') }}</h6>');
            }
            ...
            $('.datatables-basic tbody').on('change', '.change-status', function() {
                ...
                $.ajax({
                    url: '/admin/stores/{{ $store->id }}/products/' + id + '/activate',
                    type: 'PATCH',
                    data: {
                        _token: token
                    }
            ...
            $('.datatables-basic tbody').on('click', '.delete-record', function() {
                ...
                Swal.fire({
                    ...
                    if (result.value) {
                        $.ajax({
                            url: '/admin/stores/{{ $store->id }}/products/' + id,
                            type: 'POST',
                            data: {
                                _method: 'DELETE',
                                _token: token
