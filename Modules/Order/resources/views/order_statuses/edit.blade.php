    ...
    <div class="col-md-12 col-12">
        <div class="card">
            <div class="card-header">
                <h4 class="card-title">{{ __('order::general.edit_order_status') }}
                    {{ $orderStatus->getTranslation('title', 'ar') }}</h4>
            </div>
            <div class="card-body">
                <form class="form form-horizontal" action="{{ route('admin.order_status.update', $orderStatus->id) }}"
                    method="POST">
                    {{ method_field('PUT') }}
                    {{ csrf_field() }}
                    <div class="row">
                            ...
                            <div class="mb-1">
                                <div class="form-check">
                                    <input type="checkbox" value="1"
                                        {{ old('is_active', $orderStatus->is_active) ? 'checked' : '' }} name="is_active"
                        ...
                        <div class="col-sm-9 offset-sm-3">
                            <button type="submit" class="btn btn-primary me-1"><i data-feather="edit"
                                    class="me-50"></i>{{ __('order::general.edit_order_status') }}</button>
                        </div>
                    </div>
                </form>
            </div>
