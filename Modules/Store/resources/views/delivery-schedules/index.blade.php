...
@section('title', __('store::general.delivery_schedule.title'))
            ...
            <div class="col-sm-12">
                <div class="card">
                    <div class="card-header">
                        <h4 class="card-title">{{ __('store::general.delivery_schedule.title') }} - {{ $store->getTranslation('title', $locale) }}</h4>
                    </div>
                    <div class="card-body">
                        <nav aria-label="breadcrumb">
                            <ol class="breadcrumb">
                                ...
                                <li class="breadcrumb-item active" aria-current="page">
                                    {{ __('store::general.delivery_schedule.title') }}</li>
                            </ol>
                        </nav>
                    </div>
                </div>
                    ...
                    <table class="datatables-basic table">
                        <thead>
                            <tr>
                                ...
                                <th>{{ __('store::attribute.day') }}</th>
                                <th>{{ __('store::attribute.times') }}</th>
                                <th></th>
                            </tr>
                        </thead>
                    </table>
    ...
    <script>
        $(function() {
            ...
            if (dt_basic_table.length) {
                    ...
                    columnDefs: [{
                        ...
                        },
                        {
                            ...
                            render: function(data, type, full, meta) {
                                ...
                                return '<span class="text-muted">{{ __('store::general.delivery_schedule.empty') }}</span>';
                            }
                        },
                        {
                            // Actions
                            ...
                            render: function(data, type, full, meta) {
                                return (
                                    ...
                                    @can('Edit-store')
                                        +
                                        '<a href="/admin/stores/{{ $store->id }}/delivery-schedules/' + data +
                        ...
                        @can('Edit-store')
                            {
                                ...
                                }) + '{{ __('store::general.delivery_schedule.add') }}',
                                className: 'create-new btn btn-primary',
                                action: function(e, dt, node, config) {
                                    window.location.href = '{{ route('admin.store.delivery-schedules.create', $store->id) }}';
                                },
                ...
                $('div.head-label').html('<h6 class="mb-0">{{ __('store::general.delivery_schedule.title') }}</h6>');
            }
            ...
            $('.datatables-basic tbody').on('click', '.delete-record', function() {
                ...
                Swal.fire({
                    ...
                    if (result.value) {
                        $.ajax({
                            url: '/admin/stores/{{ $store->id }}/delivery-schedules/' + id,
                            type: 'POST',
                            data: {
                                _method: 'DELETE',
                                _token: token
