    ...
    <div class="col-md-12 col-12">
        <div class="card">
            <div class="card-header">
                <h4 class="card-title">{{ __('clinic::general.edit') }} {{ $product->getTranslation('title', $locale) }} - {{ $clinic->getTranslation('title', $locale) }}</h4>
            </div>
            <div class="card-body">
                <form class="form form-horizontal" action="{{ route('admin.clinic.products.update', [$clinic->id, $product->id]) }}" method="POST"
                    ...
                    <div class="row">
                        ...
                        <div class="col-12 mb-2">
                            <div class="alert alert-info mb-0" role="alert">
                                <div class="alert-body d-flex align-items-center">
                                    ...
                                    <div>
                                        <strong>{{ __('clinic::general.product_override.master_details') }}:</strong>
                                        <span>{{ __('clinic::general.product_override.notice') }}</span>
                                    </div>
                                </div>
                            ...
                            <div class="mb-1 row">
                                ...
                                <div class="col-sm-9">
                                    @if ($sellerProduct->images->isNotEmpty())
                                        <div class="mb-1">
                                            <span class="badge badge-light-primary mb-50">{{ __('clinic::general.product_override.custom_images') }}</span>
                                                ...
                                                @foreach ($sellerProduct->images as $img)
                                                    <div class="position-relative d-inline-block" id="image-wrapper-{{ $img->id }}">
                                                        ...
                                                        <button type="button" class="btn btn-icon btn-sm btn-danger position-absolute top-0 end-0 m-25 delete-seller-image"
                                                            data-id="{{ $img->id }}"
                                                            title="{{ __('clinic::general.delete') }}">
                                    ...
                                    @elseif ($product->images->isNotEmpty())
                                        <div class="mb-1">
                                            <span class="badge badge-light-secondary mb-50">{{ __('clinic::general.product_override.master_images') }}</span>
                                            <div class="d-flex flex-wrap gap-1">
                                                @foreach ($product->images as $masterImg)
                                                    <div class="d-inline-block">
                                                        <img class="rounded border width-100 height-100 cursor-pointer opacity-75" role="button"
                                                            ...
                                                            title="{{ __('clinic::general.product_override.master_images') }}">
                                                    </div>
                                                    <x-common::modal id="masterImageModal{{ $masterImg->id }}" title="{{ __('clinic::general.product_override.master_images') }}" body-class="text-center">
                        ...
                        <div class="col-sm-9 offset-sm-3">
                            ...
                            <a href="{{ route('admin.clinic.products.index', $clinic->id) }}"
                                class="btn btn-outline-secondary">{{ __('clinic::general.cancel') }}</a>
                        </div>
                    </div>
                </form>
            </div>
    ...
    <script>
        $(function() {
            ...
            $('.delete-seller-image').on('click', function() {
                ...
                var deleteUrl = "{{ route('admin.clinic.products.images.destroy', [$clinic->id, $product->id, ':image_id']) }}".replace(':image_id', imageId);
