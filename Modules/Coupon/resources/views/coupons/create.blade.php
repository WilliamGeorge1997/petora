...
@section('title', __('coupon::general.create') ?? 'Create Coupon')
    ...
    <div class="col-md-12 col-12">
        <div class="card">
            <div class="card-header">
                <h4 class="card-title">{{ __('coupon::general.create') ?? 'Create Coupon' }}</h4>
            </div>
            <div class="card-body">
                <form class="form form-horizontal" action="{{ route('admin.coupon.store') }}" method="POST">
                    {{ csrf_field() }}
                    <div class="row">
                                ...
                                <div class="col-sm-9">
                                    <select class="form-select select2" name="discount_on" id="discount_on" required>
                                        @foreach (CouponDiscountOn::options() as $value => $label)
                                            <option value="{{ $value }}" {{ old('discount_on') == $value ? 'selected' : '' }}>
                                                {{ $label }}
                                            </option>
                                        @endforeach
                                    </select>
                        ...
                        <div class="col-sm-9 offset-sm-3">
                            <button type="submit" class="btn btn-primary me-1"><i data-feather="plus"
                                    class="me-50"></i>{{ __('coupon::general.create') ?? 'Create' }}</button>
                        </div>
                    </div>
                </form>
            </div>
