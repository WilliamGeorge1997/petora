...
@section('title', __('community::general.comment.index'))
            ...
            <div class="col-sm-12">
                <div class="card">
                    <div class="card-header">
                        <h4 class="card-title">{{ __('community::general.comment.index') }}</h4>
                    </div>
                    <div class="card-body">
                        <nav aria-label="breadcrumb">
                            <ol class="breadcrumb">
                                ...
                                <li class="breadcrumb-item active" aria-current="page">
                                    {{ __('community::general.comment.index') }}</li>
                            </ol>
                        </nav>
                    </div>
                </div>
    ...
    <section class="card mb-2">
        <div class="card-body">
            <form>
                <div class="row g-2">
                    <div class="col-md-4">
                        <label class="form-label">{{ __('community::attribute.content') }}</label>
                        <input type="text" name="content" class="form-control"
                            placeholder="{{ __('community::attribute.content') }}" value="{{ request('content') }}">
                    </div>
                    <div class="col-md-3">
                    ...
                    <table class="datatables-basic table">
                        <thead>
                            <tr>
                                ...
                                <th>Client</th>
                                <th>Post ID</th>
                                <th>{{ __('community::attribute.content') }}</th>
...
@section('js')
    ...
    <script>
        $(function() {
            ...
            var ajaxRequest = "comments?";
            if (page != null) ajaxRequest += "page=" + page + '&';
            if (content != null) ajaxRequest += "content=" + content + '&';
                ...
                var dt_basic = dt_basic_table.DataTable({
                    ...
                    columnDefs: [{
                        ...
                        },
                        {
                            ...
                            render: function(data, type, full, meta) {
                                return data ? '<a href="/admin/posts/' + data + '">#' + data + '</a>' : '-';
                            }
                        },
                        {
                            // Content
                        ...
                        },
                        {
                            ...
                            render: function(data, type, full, meta) {
                                return (
                                    ...
                                    @can('Edit-comment')
                                        +
                                        '<a href="comments/' + data + '" class="dropdown-item">' +
                    ...
                    responsive: {
                        details: {
                            display: $.fn.dataTable.Responsive.display.modal({
                                header: function(row) {
                                    var data = row.data();
                                    return 'Details of Comment ' + data['id'];
                                }
                            }),
                            type: 'column',
            ...
            $('.datatables-basic tbody').on('change', '.change-status', function() {
                ...
                $.ajax({
                    url: '/admin/comments/' + id + '/activate',
                    type: 'PATCH',
                    data: {
                        _token: token
                    }
