            ...
            <div class="col-sm-12">
                <div class="card">
                    <div class="card-header">
                        <h4 class="card-title">{{ __('category::general.categories') }}</h4>
                    </div>
                    <div class="card-body">
                        <nav aria-label="breadcrumb">
                            <ol class="breadcrumb">
                                <li class="breadcrumb-item"><a
                                        href="{{ route('admin.dashboard') }}">{{ __('category::general.home') }}</a></li>
                                <li class="breadcrumb-item active" aria-current="page">
                                    {{ __('category::general.categories') }}</li>
                            </ol>
                        </nav>
                    </div>
                </div>
    ...
    <section class="card mb-2">
        <div class="card-body">
            <form>
                <div class="row g-2">
                    <div class="col-md-4">
                        <label class="form-label">{{ __('category::attribute.title') }}</label>
                        <input type="text" name="title" class="form-control"
                            placeholder="{{ __('category::attribute.title') }}" value="{{ request('title') }}">
                    </div>
                    <div class="col-md-3">
                        <label class="form-label">{{ __('category::attribute.is_active') }}</label>
                        <select name="is_active" class="form-select">
                            <option value="">{{ __('category::general.select_status') ?? 'الكل' }}</option>
                    ...
                    <div class="col-md-4 d-flex align-items-end">
                        <button type="submit" class="btn btn-primary me-1">{{ __('category::general.search') }}</button>
                        <a href="{{ route('admin.category.index') }}"
                            class="btn btn-outline-secondary">{{ __('category::general.reset') }}</a>
                    </div>
                </div>
            </form>
        </div>
    ...
    <script>
        $(function() {
            ...
            if (dt_basic_table.length) {
                var dt_basic = dt_basic_table.DataTable({
                    ...
                    columnDefs: [{
                        ...
                        },
                        {
                            // Actions
                            targets: -1,
                            title: '{{ __('category::general.actions') }}',
                            orderable: false,
                            render: function(data, type, full, meta) {
                                return (
                ...
                $('div.head-label').html('<h6 class="mb-0">{{ __('category::general.main_data') }}</h6>');
            }
            ...
            $('.datatables-basic tbody').on('click', '.delete-record', function() {
                ...
                var id = dt_basic.row($(this).parents('tr')).data().id;
                Swal.fire({
                    title: '{{ __('category::general.sure_delete') }}',
                    text: '{{ __('category::general.cant_revert') }}',
                    icon: 'warning',
                    showCancelButton: true,
                    confirmButtonText: '{{ __('category::general.yes_delete') }}',
                    cancelButtonText: '{{ __('category::general.cancel') }}',
                    customClass: {
