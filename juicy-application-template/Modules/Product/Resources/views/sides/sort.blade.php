    ...
    <section>
        <div class="card">
            <div class="card-header">
                <h4 class="card-title mb-0">ترتيب الاطباق الجانبية</h4>
            </div>
            <div class="card-body">
                ...
                @endif
                @if ($sides->isEmpty())
                    <p class="text-center alert alert-info">لا توجد اطباق جانبية.</p>
                @else
                    <form method="POST" action="{{ route('sides.sort.update') }}">
                        @csrf
                        <div id="side-sort-list">
                            @foreach ($sides as $side)
                                <div class="side-sort-item" data-id="{{ $side->id }}">
                                    <span class="drag-handle me-50">☰</span>
                                    <span class="side-order-indicator me-50">{{ $loop->iteration }}</span>
                                    <span class="side-name">
                                        {{ $side->getTranslations('title')['ar'] }}
                                        -
                                        {{ $side->getTranslations('title')['en'] }}
                                    </span>
                                    <input type="hidden" name="sort_order[]" value="{{ $side->id }}">
                                </div>
                            @endforeach
...
@section('js')
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            const list = document.getElementById('side-sort-list');
            if (list) {
                new Sortable(list, {
                    animation: 150,
                    onEnd: function() {
                        const items = list.querySelectorAll('.side-sort-item');
                        items.forEach((el, index) => {
                            el.querySelector('.side-order-indicator').textContent = index + 1;
