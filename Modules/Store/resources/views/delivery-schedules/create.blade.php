...
@section('title', __('store::general.delivery_schedule.add'))
    ...
    <div class="col-md-12 col-12">
        <div class="card">
            <div class="card-header">
                <h4 class="card-title">{{ __('store::general.delivery_schedule.details') }} - {{ $store->getTranslation('title', $locale) }}</h4>
            </div>
            <div class="card-body">
                <form class="form form-horizontal" action="{{ route('admin.store.delivery-schedules.store', $store->id) }}" method="POST">
                    {{ csrf_field() }}
                    <div class="row">
                            ...
                            <div class="mb-1 row">
                                <div class="col-sm-3 text-center">
                                    <label class="col-form-label" for="daySelect">
                                        {{ __('store::general.delivery_schedule.select') }} <span class="text-danger">*</span>
                                    </label>
                                </div>
                                <div class="col-sm-9">
                                    <select class="form-select select2 @error('day') is-invalid @enderror" id="daySelect" name="day" required>
                                        <option value="" disabled selected>{{ __('store::general.delivery_schedule.select') }}</option>
                                        @foreach ($days as $dayKey => $dayLabel)
                                            @php
                                                $isAlreadyAdded = in_array($dayKey, $existingDays);
                                            @endphp
                                            <option value="{{ $dayKey }}" {{ $isAlreadyAdded ? 'disabled' : '' }} {{ old('day') === $dayKey ? 'selected' : '' }}>
                                                {{ $dayLabel }} {{ $isAlreadyAdded ? '(' . __('store::general.delivery_schedule.exists') . ')' : '' }}
                                            </option>
                                        @endforeach
                                    </select>
                                    @error('day')
                            ...
                            <div class="mb-1 row">
                                <div class="col-sm-3 text-center">
                                    <label class="col-form-label">
                                        {{ __('store::general.delivery_schedule.shifts') }} <span class="text-danger">*</span>
                                    </label>
                                </div>
                                <div class="col-sm-9">
                                        ...
                                        <div class="row">
                                            <div class="col-12">
                                                <button type="button" class="btn btn-sm btn-outline-primary" data-repeater-create>
                                                    <i data-feather="plus" class="me-50"></i> {{ __('store::general.delivery_schedule.add_shift') }}
                                                </button>
                                            </div>
                                        </div>
                                    </div>
                        ...
                        <div class="col-sm-9 offset-sm-3">
                            <button type="submit" class="btn btn-primary me-1"><i data-feather="plus" class="me-50"></i>{{ __('store::general.delivery_schedule.add') }}</button>
                            <a href="{{ route('admin.store.delivery-schedules.index', $store->id) }}" class="btn btn-outline-secondary">{{ __('store::general.cancel') }}</a>
                        </div>
                    </div>
                </form>
            </div>
            ...
            $('.repeater-default').repeater({
                ...
                hide: function (deleteElement) {
                    if ($('[data-repeater-item]').length > 1) {
                        ...
                        Swal.fire({
                            text: '{{ __('store::general.delivery_schedule.min') }}',
                            icon: 'info',
