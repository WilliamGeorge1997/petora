...
@section('title', __('order::general.order_methods'))
...
@section('content')
            ...
            <div class="col-sm-12">
                <div class="card">
                    <div class="card-header">
                        <h4 class="card-title">{{ __('order::general.order_methods') }}</h4>
                    </div>
                    <div class="card-body">
                        <nav aria-label="breadcrumb">
                            <ol class="breadcrumb">
                                ...
                                <li class="breadcrumb-item active" aria-current="page">
                                    {{ __('order::general.order_methods') }}</li>
                            </ol>
                        </nav>
                    </div>
                </div>
    ...
    <section class="card mb-2">
        <div class="card-body">
            <form>
                <div class="row g-2">
                    <div class="col-md-4">
                        <label class="form-label">{{ __('order::attribute.title') }}</label>
                        <input type="text" name="title" class="form-control"
                            placeholder="{{ __('order::attribute.title') }}" value="{{ request('title') }}">
                    </div>
                    <div class="col-md-3">
                        <label class="form-label">{{ __('order::attribute.is_active') }}</label>
                        <select name="is_active" class="form-select">
                            <option value="">{{ __('order::general.select_status') ?? 'الكل' }}</option>
                    ...
                    <div class="col-md-4 d-flex align-items-end">
                        ...
                        <a href="{{ route('admin.order_method.index') }}"
                            class="btn btn-outline-secondary">{{ __('order::general.reset') }}</a>
                    </div>
                </div>
            </form>
        </div>
                    ...
                    <table class="datatables-basic table">
                        <thead>
                            <tr>
                                <th></th>
                                <th></th>
                                <th>{{ __('order::attribute.id') }}</th>
                                <th>{{ __('order::attribute.title') }}</th>
                                <th>{{ __('order::attribute.created_at') }}</th>
                                <th>{{ __('order::attribute.is_active') }}</th>
                                <th></th>
                            </tr>
                        </thead>
                    </table>
    ...
    <!--/ Basic table -->
    {{ $orderMethods->withQueryString()->links() }}
@endsection
        ...
        $(function() {
            ...
            if (dt_basic_table.length) {
                var dt_basic = dt_basic_table.DataTable({
                        ...
                        },
                        {
                            ...
                            render: function(data, type, full, meta) {
                                return (
                                    ...
                                    '</div>' +
                                    '</div>'
                                    @can('Edit-ordermethod')
                                        +
                                        '<a href="order-methods/' + data +
                    ...
                    bPaginate: false,
                    buttons: [
                        @can('Create-ordermethod')
                            {
                                text: feather.icons['plus'].toSvg({
                                    class: 'me-50 font-small-4'
                                }) + '{{ __('order::general.create_order_method') }}',
                                className: 'create-new btn btn-primary',
                                action: function(e, dt, node, config) {
                                    window.location.href = './order-methods/create';
                                },
            ...
            $('.datatables-basic tbody').on('change', '.change-status', function() {
                ...
                $.ajax({
                    url: '/admin/order-methods/' + id + '/activate',
                    type: 'PATCH',
                    data: {
                        _token: token
                    }
