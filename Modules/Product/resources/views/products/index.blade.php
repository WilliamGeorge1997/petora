            ...
            <div class="col-sm-12">
                <div class="card">
                    <div class="card-header">
                        <h4 class="card-title">{{ __('product::general.products') }}</h4>
                    </div>
                    <div class="card-body">
                        <nav aria-label="breadcrumb">
                            <ol class="breadcrumb">
                                <li class="breadcrumb-item"><a
                                        href="{{ route('admin.dashboard') }}">{{ __('common::general.home') ?? 'الرئيسية' }}</a></li>
                                <li class="breadcrumb-item active" aria-current="page">
                                    {{ __('product::general.products') }}</li>
                            </ol>
                        </nav>
                    </div>
                </div>
    ...
    <section class="card mb-2">
        <div class="card-body">
            <form>
                <div class="row g-2">
                    <div class="col-md-4">
                        <label class="form-label">{{ __('product::attribute.title') ?? 'الاسم' }}</label>
                        <input type="text" name="title" class="form-control"
                            placeholder="{{ __('product::attribute.title') ?? 'الاسم' }}" value="{{ request('title') }}">
                    </div>
                    <div class="col-md-3">
                    ...
                    <div class="col-md-4 d-flex align-items-end">
                        <button type="submit" class="btn btn-primary me-1">{{ __('common::general.search') ?? 'بحث' }}</button>
                        <a href="{{ route('admin.product.index') }}"
                            class="btn btn-outline-secondary">{{ __('common::general.reset') ?? 'إعادة ضبط' }}</a>
                    </div>
                </div>
            </form>
        </div>
    ...
    <script>
        $(function() {
            ...
            if (dt_basic_table.length) {
                var dt_basic = dt_basic_table.DataTable({
                    ...
                    columnDefs: [{
                        ...
                        },
                        {
                            // Actions
                            targets: -1,
                            title: '{{ __('product::general.actions') }}',
                            orderable: false,
                            render: function(data, type, full, meta) {
                                return (
                        ...
                        details: {
                            ...
                            renderer: function(api, rowIdx, columns) {
                                var data = $.map(columns, function(col, i) {
                                    return col.title !==
                                        '' // ? Do not show row in popup if title is blank
                                        ?
                                        '<tr data-dt-row="' +
                                        col.rowIdx +
                                        '" data-dt-column="' +
                ...
                $('div.head-label').html('<h6 class="mb-0">{{ __('product::general.main_data') }}</h6>');
            }
            ...
            $('.datatables-basic tbody').on('click', '.delete-record', function() {
                ...
                var id = dt_basic.row($(this).parents('tr')).data().id;
                Swal.fire({
                    title: '{{ __('product::general.sure_delete') }}',
                    text: '{{ __('product::general.cant_revert') }}',
                    icon: 'warning',
                    showCancelButton: true,
                    confirmButtonText: '{{ __('product::general.yes_delete') }}',
                    cancelButtonText: '{{ __('product::general.cancel') }}',
                    customClass: {
