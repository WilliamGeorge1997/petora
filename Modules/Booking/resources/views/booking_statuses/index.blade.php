...
@section('title', __('booking::general.status.index'))
...
@section('content')
            ...
            <div class="col-sm-12">
                <div class="card">
                    <div class="card-header">
                        <h4 class="card-title">{{ __('booking::general.status.index') }}</h4>
                    </div>
                    <div class="card-body">
                        <nav aria-label="breadcrumb">
                            <ol class="breadcrumb">
                                <li class="breadcrumb-item"><a
                                        href="{{ route('admin.dashboard') }}">{{ __('common::general.home') }}</a></li>
                                <li class="breadcrumb-item active" aria-current="page">
                                    {{ __('booking::general.status.index') }}</li>
                            </ol>
                        </nav>
                    </div>
                </div>
    ...
    <section class="card mb-2">
        <div class="card-body">
            <form>
                <div class="row g-2">
                    <div class="col-md-4">
                        <label class="form-label">{{ __('booking::attribute.title') }}</label>
                        <input type="text" name="title" class="form-control"
                            placeholder="{{ __('booking::attribute.title') }}" value="{{ request('title') }}">
                    </div>
                    <div class="col-md-3">
                        <label class="form-label">{{ __('booking::attribute.is_active') }}</label>
                        <select name="is_active" class="form-select">
                            <option value="">{{ __('common::general.select_status') ?? 'الكل' }}</option>
                    ...
                    <div class="col-md-4 d-flex align-items-end">
                        <button type="submit" class="btn btn-primary me-1">{{ __('common::general.search') }}</button>
                        <a href="{{ route('admin.booking_status.index') }}"
                            class="btn btn-outline-secondary">{{ __('common::general.reset') }}</a>
                    </div>
                </div>
            </form>
        </div>
                    ...
                    <table class="datatables-basic table">
                        <thead>
                            <tr>
                                <th></th>
                                <th></th>
                                <th>{{ __('booking::attribute.id') }}</th>
                                <th>{{ __('booking::attribute.title') }}</th>
                                <th>{{ __('booking::attribute.created_at') }}</th>
                                <th>{{ __('booking::attribute.is_active') }}</th>
                                <th></th>
                            </tr>
                        </thead>
                    </table>
    ...
    <!--/ Basic table -->
    {{ $bookingStatuses->withQueryString()->links() }}
@endsection
...
@section('js')
    ...
    <script>
        $(function() {
            ...
            var ajaxRequest = "booking-statuses?";
            ...
            if (dt_basic_table.length) {
                var dt_basic = dt_basic_table.DataTable({
                    ...
                    columnDefs: [{
                        ...
                        },
                        {
                            ...
                            render: function(data, type, full, meta) {
                                ...
                                var $state = states[stateNum] ? states[stateNum] : 'primary',
                        ...
                        },
                        {
                            ...
                            render: function(data, type, full, meta) {
                                return (
                                    ...
                                    '</div>' +
                                    '</div>'
                                    @can('Edit-bookingstatus')
                                        +
                                        '<a href="booking-statuses/' + data +
                    ...
                    bPaginate: false,
                    buttons: [
                        @can('Create-bookingstatus')
                            {
                                text: feather.icons['plus'].toSvg({
                                    class: 'me-50 font-small-4'
                                }) + '{{ __('booking::general.status.create') }}',
                                className: 'create-new btn btn-primary',
                                action: function(e, dt, node, config) {
                                    window.location.href = './booking-statuses/create';
                                },
                ...
                $('div.head-label').html('<h6 class="mb-0">{{ __('common::general.main_data') }}</h6>');
            }
            ...
            $('.datatables-basic tbody').on('change', '.change-status', function() {
                ...
                $.ajax({
                    url: '/admin/booking-statuses/' + id + '/activate',
                    type: 'PATCH',
                    data: {
                        _token: token
                    }
